  ...
  const methods = {
    ...
    toQuery() {
      ...
      if (crud.query.createTime) {
        const createTimeArr = crud.query.createTime
        crud.query['startTime'] = createTimeArr[0]
        crud.query['endTime'] = createTimeArr[1]
        delete crud.query.createTime
      }
      crud.refresh()
    },
    // 刷新
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          if (table && (
            table.$attrs.tablename === 'menu' ||
            table.$attrs.tablename === 'assist'
          )
          ) {
            crud.page.total = 1
            crud.data = data.data
     ...
     */
    submitCU() {
      // console.log('提交', !callVmHook(crud, CRUD.HOOK.beforeValidateCU))
    ...
    attchTable() {
      ...
      table.$on('expand-change', (row, expanded) => {
        if (!expanded || (table.$attrs && table.$attrs.tablename === 'menu')) {
          return
        }
